...
export default function ProductEditClient({ product, categories, types, colors: initialColors }: ProductEditClientProps) {
  ...
  // ✅ Cargar colores existentes del producto
  useEffect(() => {
    if (product.product_colors?.length > 0) {
      const existingColorIds = product.product_colors.map((pc: any) => pc.color_id)
      setColorIds(existingColorIds)
    } else if (product.colors?.length > 0) {
      const existingColorIds = product.colors.map((c: any) => c.id)
      setColorIds(existingColorIds)
    }
  }, [product])
  ...
  // ✅ Handler para agregar color nuevo
  const handleColorAdded = useCallback(async (colorId?: string) => {
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="border-t pt-6">
              <ColorSelector
                ...
                onAddNew={() => setShowColorModal(true)}
                disabled={loading}
              />
            </div>
            ...
            {/* Switches */}
            <div className="flex flex-col sm:flex-row gap-4 p-4 bg-gray-50 rounded-lg border-t">
